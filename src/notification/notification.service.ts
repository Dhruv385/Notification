...
export class NotificationService {
  ...
  generateNotificationMessage(
    ...
    switch (action) {
      case 'comment':
        return `${username ?? 'Someone'} commented on this post: ${postId}`;
      case 'like':
        return `${username ?? 'Someone'} liked this post ${postId}`;
      case 'reply':
        return `${username ?? 'Someone'} replied on this comment ${parentCommentId}}`;
      default:
